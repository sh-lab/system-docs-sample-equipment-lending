...
## 1. サービスID・名称
- ID：`HFP-EL-SCS003_confirm-rejected-request_service`
- 名称：`却下確認完了コマンドサービス`

---

## 2. 役割と責務
- 本サービスは、利用者本人による却下内容確認完了の登録を担当する。
- 対象申請の取得、所有者確認、状態遷移可否確認、楽観ロック確認、完了情報の更新、対応する履歴テーブルへの操作履歴登録を行う。
...
3. 対象申請の現在状態が `REJECTED` であること、および `VERSION` が入力値と一致することを確認する。
4. エンティティの状態を `COMPLETED` へ変更し、完了日時と監査項目を設定する。
...
- `コマンドサービスID` には `HFP-EL-SCS003_confirm-rejected-request_service` を設定する。
- 同一 `operationId` で複数エンティティを更新しないサービスであるため、履歴行は常に 1 件である。

---

## 8. 例外方針
- 業務例外：対象申請が存在しない、本人以外の申請である、却下状態でない、またはバージョン不一致の場合は更新不可として送出する。
