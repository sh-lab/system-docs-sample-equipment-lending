...
- 貸出状態の許可値を `ALL`、`AVAILABLE`、`UNAVAILABLE` に制御し、不正値は既定値 `AVAILABLE` に補正する。
...
3. `lendingStatus` が `ALL`、`AVAILABLE`、`UNAVAILABLE` 以外の場合は `AVAILABLE` を適用する
...
| lendingStatus | string | 任意 | `ALL`、`AVAILABLE`、`UNAVAILABLE` のみ許可する |
