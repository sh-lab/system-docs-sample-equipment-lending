...
## 1. サービスID・名称
- ID：`HFP-EL-SCS011_reject-lending-request_service`
- 名称：`申請却下コマンドサービス`

---

## 2. 役割と責務
- 本サービスは、承認待ちの貸出申請の却下処理を担当する。
- 対象申請の取得、状態遷移可否確認、楽観ロック確認、却下情報の更新、対象備品状態の差し戻し、対応する履歴テーブルへの操作履歴登録を行う。
...
## 3. 目的・スコープ
- 目的：承認待ちの貸出申請を却下状態へ更新し、対象備品を貸出可能状態へ戻す。
...
### 4.2 事後条件
- 正常終了時：
  - 対象申請の状態が `REJECTED` に更新される。
  - 却下者ユーザーID、管理者コメント、却下日時が記録される。
...
5. 申請エンティティの状態を `REJECTED` へ変更し、却下者ユーザーID、管理者コメント、却下日時、監査項目を設定する。
...
## 8. 例外方針
- 業務例外：対象申請が存在しない、承認待ちでない、またはバージョン不一致の場合は更新不可として送出する。
...
- `コマンドサービスID` には `HFP-EL-SCS011_reject-lending-request_service` を設定する。
